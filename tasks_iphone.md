...
| 13 | DONE | 2026-03-07 | **Edge-to-edge full-screen fix.** Root cause: auto-generated Info.plist had NO `UILaunchScreen` dictionary — Xcode 17's `INFOPLIST_KEY_UILaunchScreen_*` build settings did not generate it. Without `UILaunchScreen`, iOS runs the app in compatibility/letterbox mode at a reduced viewport. This is why `.ignoresSafeArea()` never worked across 4 prior sessions — the app was not filling the screen at all. Fix: created explicit `GreatnessSimulator/Info.plist` with `UILaunchScreen` dict, set `GENERATE_INFOPLIST_FILE = NO` and `INFOPLIST_FILE = GreatnessSimulator/Info.plist` in pbxproj. Discovered by comparing with working full-screen projects (forest, tourbuddy, hnefatafl) which all had proper UILaunchScreen declarations. Also discovered xcodebuild outputs to `~/Library/Developer/Xcode/DerivedData/` not the project's `build/` dir — stale builds gave false negatives. |
